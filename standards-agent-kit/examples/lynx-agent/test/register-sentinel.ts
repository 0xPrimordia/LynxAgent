...
/**
 * Load the Sentinel logo image and convert to base64
 */
async function loadLogoImage(): Promise<string> {
  try {
    // Define the path to the logo
    const logoPath = path.join(__dirname, '..', 'assets', 'sentinel-logo.png');
...
async function main() {
  logger.info(`=== Registering Sentinel Agent (${validNetworkType}) ===`);
  ...
  try {
    ...
    // Set the current agent with sentinel credentials
    stateManager.setCurrentAgent({
      name: 'Sentinel Agent',
      accountId: process.env.SENTINEL_ACCOUNT!,
    ...
    // Create RegisterAgentTool (similar to standards-expert example)
    ...
    // Load the logo image
    logger.info('Loading Sentinel logo...');
    const profilePicture = await loadLogoImage();
    
    // Registration parameters
    const registrationParams = {
      name: 'Sentinel Agent',
      description: 'Market monitoring agent that detects price changes and sends alerts',
      capabilities,
    ...
    // Register the agent
    logger.info('Registering Sentinel Agent using RegisterAgentTool...');
    ...
    if (result.success) {
      ...
      // Update .env file if it exists (similar to standards-expert example)
      ...
      const envVars = {
        SENTINEL_ACCOUNT_ID: result.accountId,
        SENTINEL_PRIVATE_KEY: result.privateKey,
        SENTINEL_INBOUND_TOPIC_ID: result.inboundTopicId,
        SENTINEL_OUTBOUND_TOPIC_ID: result.outboundTopicId,
        SENTINEL_PROFILE_TOPIC_ID: result.profileTopicId || '',
      };
      ...
      // Display next steps
      logger.info('\n=== NEXT STEPS ===');
      logger.info('1. Update your .env or .env.local file with the new SENTINEL credentials');
      logger.info('2. Start the Sentinel Agent with npm run lynx-agent:start-sentinel');
    } else {
      logger.error('\nFailed to register agent:', result.message || resultJson);
    }
  } catch (error) {
    logger.error('Error registering Sentinel Agent:', error);
